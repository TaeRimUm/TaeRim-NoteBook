...
class RUN:
    ...
    def do(self):
        self.frame = (self.frame + FRAMES_PER_ACTION * game_framework.frame_time) % 8
        self.x += self.dir * RUN_SPEED_PPS * game_framework.frame_time
        self.x = clamp(25, self.x, 1600-25)

    def draw(self):
        if self.dir == -1:
            self.image.clip_composite_draw(int(self.frame)*100, 0, 100, 100, rad, flip, self.x, self.y, w ,h)
        elif self.dir == 1:
            self.image.clip_draw(int(self.frame)*100, 100, 100, 100, self.x, self.y)
...
class SLEEP:
    ...
    def draw(self):
        if self.face_dir == -1:
            self.image.clip_draw(int(self.frame) * 100, 200, 100, 100,
                                          -3.141592 / 2, '', self.x + 25, self.y - 25, 100, 100)
        else:
            self.image.clip_draw(int(self.frame) * 100, 300, 100, 100,
                                          3.141592 / 2, '', self.x - 25, self.y - 25, 100, 100)
